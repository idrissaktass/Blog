...
function Book() {
  const { bookId } = useParams(); // Get the recipe ID from URL parameter
  ...
  useEffect(() => {
    const fetchBook = async () => {
      try {
        ...
        console.error("Error fetching book advice details:", error);
      }
    };
  ...
  return (
    <div className="row justify-content-center">
      <div className="col-7 details">
        <img src="" alt="" />
        <h2 className="h-details">{book.title}</h2>
        <p>{book.summary}</p>
        ...
        <ul className="ingredients-list">
          {book.positive.map((positive, index) => (
        ...
        <ul className="recipe-list">
          {book.negative.map((content, index) => (
            <li key={index}>{content}</li>
          ))}
        </ul>
        <p className="name-details">User Name: {book.username}</p>
      </div>
    </div>
  );
}
